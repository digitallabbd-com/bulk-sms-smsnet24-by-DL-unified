...
final class SchoolManagementHooks {
    public function register(): void {
        // Example hooks; adapt per plugin version.
        add_action('smgt_after_add_student', [$this, 'on_student_added'], 10, 1);
        add_action('smgt_after_publish_exam_result', [$this, 'on_result_published'], 10, 2);
        add_action('smgt_after_attendance_marked', [$this, 'on_attendance_marked'], 10, 2);
    }
    ...
    public function on_student_added($student_id): void {
        ...
        (new Sender())->queue($phone, $msg, ['event' => 'sm_student_added', 'student_id' => (int) $student_id]);
    }
    ...
    public function on_result_published($exam_id, $student_id): void {
        ...
        (new Sender())->queue($phone, $msg, ['event' => 'sm_result_published', 'student_id' => (int) $student_id, 'exam_id' => (int) $exam_id]);
    }

    public function on_attendance_marked($date, $absentees): void {
        $opts = get_option(SettingsPage::OPT, []);
